...
class SearchTest {
    ...
    void testMatch() throws IOException {
        ...
        SearchRequest request = new SearchRequest("disable-date-requirements");
        // 2.准备请求参数
        request.source().query(QueryBuilders.termQuery("personId", "1"));
        // 3.发送请求，得到响应
        SearchResponse response = client.search(request, RequestOptions.DEFAULT);
        // 4.结果解析
        handleResponse(response);
...
//            // 2）根据字段名，获取高亮结果
//            HighlightField highlightField = map.get("marryStatus");
//            // 3）获取高亮结果字符串数组中的第1个元素
//            String marryStatus = highlightField.getFragments()[0].toString();
//            // 4）把高亮结果放到HotelDoc中
//            requirement.setMarryStatus(marryStatus);
            // 4.7.打印
